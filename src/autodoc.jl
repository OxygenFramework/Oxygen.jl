    ...
    return function(path = nothing; 
        ...
        return function(httpmethod::String)
            ...
            if !(isnothing(routermiddleware) && isnothing(middleware))
                ...
                ctx.custommiddleware["$httpmethod|$path"] = (routermiddleware, middleware)
            end
            ...
            if !isnothing(interval) && interval >= 0.0
                ...
                if isnothing(findfirst(x -> x === task, ctx.repeattasks))
                    push!(ctx.repeattasks, task)
                end
            end
            ...
            # register tags
            if !haskey(ctx.taggedroutes, path)
                ctx.taggedroutes[path] = TaggedRoute([httpmethod], combinedtags)
            else 
                combinedmethods = vcat(httpmethod, ctx.taggedroutes[path].httpmethods)
...
function registerschema(ctx::Context, path::String, httpmethod::String, parameters, returntype::Array)
    ...
    for folder in ctx.mountedfolders
    ...
    if haskey(ctx.taggedroutes, path) && httpmethod in ctx.taggedroutes[path].httpmethods
        tags = ctx.taggedroutes[path].tags 
    else 
        tags = []
    end
    ...
    mergeschema(ctx.schema, cleanedpath, route)
end
