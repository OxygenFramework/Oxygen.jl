...
using ..Types: TaggedRoute, TaskDefinition, CronDefinition, Nullable, LifecycleMiddleware,
    MCPConfig, MCPMetadata, normalize_mcp_config
...
Passing `mcp` attaches MCP metadata that every route in the group inherits:
`mcp = false` excludes the group, `mcp = true` exposes each route with defaults,
and a `NamedTuple`/`Dict` (e.g.
`(description = "User management", parameters = Dict(:id => "User ID"))`)
provides defaults that individual routes can override. A router description acts
as a group prefix for each route's tool description.
"""
function router(ctx::ServerContext, prefix::String="";
    ...
    # normalize the (optional) router level MCP metadata that routes inherit
    router_mcp = normalize_mcp_config(mcp)

    return OuterRouter(ctx, prefix, tags, router_middleware, interval, cron, router_mcp)
end
...
function (outer::OuterRouter)(
    ...
    # `nothing` means "unspecified" so the route inherits the router's metadata
    route_mcp = normalize_mcp_config(mcp)

    return InnerRouter(outer.ctx, outer, path, tags, processed_middleware, interval, cron, route_mcp)
end
