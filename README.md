...
## Autogenerated OpenAPI 3.0 Docs with Swagger
...
Oxygen can automatically generate OpenAPI return type specificiations for any primitive, vector, or struct.  It cannot do this
for dictionary types, so any top level or child fields of this type will be simply `object` in the generated schema.

**Note:** Using the `json()` render function will cause no spefic output content type to be emitted (same for `text()`, `html()` etc.)
They will still return the standard `200` and `500` high level descriptors, just without content specific information.
...
struct Project
    name::String 
    yearended::Union{Int, Nothing} # Defined as nullable integer
    contributors::Union{Vector{Person}, Nothing} # Defined as nullable Person array 
    attributes::Dict # Defined as `object`
end

# Will serialize the vector correctly
@get "/projects" function(req::HTTP.Request)
    return [Project("Project A", 2010, [Person("Albert Brown")], Dict("cost" => "\$100"))]
end

# The return type will be `application/json` with a plain `object`
@get "/data" function(req::HTTP.Request)
    return Dict("message" => "hello!", "value" => 99.3)
end
