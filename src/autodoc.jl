...
function registerschema(
    ...
    ##### Build a response payload (application/json only)
    return_types = []
    ...
    # Remove generic HTTP response objects (generated by `text()` and `html()` functions)
    # as we cannot infer the type of the serialized objects within their payloads
    ...
    # Remove `nothing` as `buildschema()` returns this if field is unserializable
    return_type_schemas = unique(filter(x -> !isnothing(x), map(x -> buildschema!(x, schemas), filtered)))
    json_response_schema = Dict()
    
    # If our function returns multiple types, then make this an `anyOf` collection
    ...
    # Build response object and append specific types if we can infer them
    responses = Dict(
