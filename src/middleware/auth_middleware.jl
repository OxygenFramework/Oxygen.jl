...
"""
function BearerAuth(validate_token::Function; scheme::String = "Bearer")
    ...
    return function (handle::Function)
        return function(req::HTTP.Request)
            ...
            auth_header = HTTP.header(req, "Authorization", missing)
            if ismissing(auth_header) || !startswith(auth_header, "Bearer ")
                return INVALID_HEADER
            end

            # Fast, allocation-free extraction of the token
